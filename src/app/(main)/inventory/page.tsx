...
export default function InventoryPage() {
  const { items, toggleAvailability, fetchMenus } = useMenuStore();
  const [search, setSearch] = useState("");

  // Auto-fetch saat mount kalo items kosong
  useEffect(() => {
    if (items.length === 0) {
      fetchMenus();
    }
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            ...
            <div className="divide-y divide-border">
              {filteredItems.map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-[auto_1fr_auto] gap-4 p-4 items-center hover:bg-secondary/10 transition-colors"
                >
                  {/* Status Indicator */}
